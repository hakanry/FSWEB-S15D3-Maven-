...
public class WordCounter {
    public static Map<String,Integer> calculatedWord(){
        ...
        String verilenTEXT = "When the offensive resumed, the Turks received their first victory when the Greeks encountered stiff resistance in the battles of First and Second İnönü," +
                ...
                " June–July 1921 saw heavy fighting in the Battle of Kütahya-Eskişehir. While it was an eventual Greek victory," +
                " the Turkish army withdrew in good order to the Sakarya river, their last line of defence." +
        ...
        Map<String,Integer> frekans = new LinkedHashMap<>();

        String isgorucelekTEXT = verilenTEXT.replaceAll("[,.-]"," ").toLowerCase();
        String[] kelimelerDizisi = isgorucelekTEXT.split("\\s+");

        List<String> kelimelerListesi = new ArrayList<>(Arrays.asList(kelimelerDizisi));

        System.out.println(kelimelerListesi);
        System.out.println(kelimelerListesi.size());
        for(String kelime : kelimelerListesi){
            if(!frekans.containsKey(kelime)){
                frekans.put(kelime,Collections.frequency(kelimelerListesi,kelime));
